        ...
        <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '8px', alignItems: 'center' }}>
          ...
          {onUpdatePortfolios && (
            <>
              <button
                onClick={() => {
                  setShowNewPortfolioForm(!showNewPortfolioForm);
                  setShowManagePortfolios(false);
                }}
                style={{
                  background: 'var(--bg3)',
              ...
              <button
                onClick={() => {
                  setShowManagePortfolios(!showManagePortfolios);
        ...
        <div className="animate-fade" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {customPortfolios.map(pName => (
              <div key={pName} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg3)', padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <span style={{ fontSize: '12px', fontWeight: 'bold', fontFamily: 'var(--font-mono)' }}>{pName.toUpperCase()}</span>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    onClick={async () => {
                      ...
                      const updatedPortfolios = customPortfolios.map(p => p === pName ? trimmed : p);
                    ...
                    style={{ background: 'var(--bg2)', border: '1px solid var(--border)', color: 'var(--text2)', padding: '4px 8px', borderRadius: '4px', fontSize: '10px', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}
                  >
                    RINOMINA
                  </button>
                  <button
                    onClick={async () => {
                      if (confirm(`Sei sicuro di voler eliminare il portafoglio "${pName}"? Tutti gli asset al suo interno verranno spostati nel portafoglio "Principale".`)) {
                        ...
                        const updatedPortfolios = customPortfolios.filter(p => p !== pName);
                        if (onUpdatePortfolios) {
                          await onUpdatePortfolios(updatedPortfolios);
                        }
                        ...
                        setSelectedPortfolio('Tutti');
                      }
                    }}
                    style={{ background: '#ef444422', border: '1px solid #ef444444', color: '#ef4444', padding: '4px 8px', borderRadius: '4px', fontSize: '10px', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}
                  >
                    ELIMINA
                  </button>
                </div>
